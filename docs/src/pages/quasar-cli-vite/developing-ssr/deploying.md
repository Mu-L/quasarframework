...
When deploying as a SPA or PWA, the distributables folder that gets generated by Quasar CLI can be served by a static webserver. However, this not the case with a SSR (Server-side Render) build. Your distributables, in this case, contain your production webserver too.
...
So what you need to do when deploying is to copy this distributables folder on your server, yarn/pnpm/bun/npm install the dependencies inside it, then run the `start` script. This boots up the webserver and starts listening for connections.
